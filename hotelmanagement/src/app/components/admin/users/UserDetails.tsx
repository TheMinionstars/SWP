import { Descriptions, Image, Modal, Result, Skeleton, Tag } from 'antd'
import React from 'react'
import { useAppDispatch } from '@/app/hooks/reduxHooks'
import { userRoleIdAsResponse, userGenderAsResponse } from '../../../utils/responseAsStatus'
import { AccountDTO } from '@/app/data/account'
import useFetchData from '@/app/hooks/useFetchData'
const { confirm } = Modal

type UserDetailsProps = {
  id: string
}
const default_account: AccountDTO = {
  id: 1,
  firstName: 'John',
  lastName: 'Doe',
  email: 'johndoe@example.com',
  password: 'password123',
  avatar: '/default.jpg',
  sex: 'Male',
  phoneNum: '1234567890',
  address: '123 Street, City',
  roleId: 1
}
const UserDetails: React.FC<UserDetailsProps> = ({ id }) => {
  const dispatch = useAppDispatch()
  // const user = getSessionUser()
  // fetch user-details API data
  const [loading, error, response] = useFetchData(`/account/${id}`)
  const account: AccountDTO = response ? response : default_account

  return (
    <Skeleton loading={loading} paragraph={{ rows: 10 }} active avatar>
      {error ? (
        <Result title='Failed to fetch' subTitle={error} status='error' />
      ) : (
        <Descriptions title='User Information' bordered>
          <Descriptions.Item label='Avatar' span={3}>
            <Image
              style={{ width: '100px', height: '100px' }}
              src={account ? account.avatar : 'error'}
              fallback='data:image/png;base64,'
              crossOrigin='anonymous'
              alt='user-image'
            />
          </Descriptions.Item>

          <Descriptions.Item label='First Name'>{account ? account.firstName : 'Default First Name'}</Descriptions.Item>
          <Descriptions.Item label='Last Name' span={2}>
            {account ? account.lastName : 'Default Last Name'}
          </Descriptions.Item>
          <Descriptions.Item label='Email'>{account ? account.email : 'Default email'}</Descriptions.Item>
          <Descriptions.Item label='Phone' span={2}>
            {account ? account.phoneNum : 'Default phone'}
          </Descriptions.Item>
          <Descriptions.Item label='Gender'>
            <Tag
              style={{ width: '80px', textAlign: 'center', textTransform: 'capitalize' }}
              color={userGenderAsResponse(account.sex).color}
            >
              {userGenderAsResponse(account.sex).level}
            </Tag>
          </Descriptions.Item>
          <Descriptions.Item label='Role' span={3}>
            <Tag
              style={{ width: '80px', textAlign: 'center', textTransform: 'capitalize' }}
              color={userRoleIdAsResponse(account.roleId).color}
            >
              {userRoleIdAsResponse(account.roleId).level}
            </Tag>
          </Descriptions.Item>
          <Descriptions.Item label='Address' span={3}>
            {account ? account.address : 'Default address'}
          </Descriptions.Item>
        </Descriptions>
      )}
    </Skeleton>
  )
}

const Index = React.memo(UserDetails)
export default Index
